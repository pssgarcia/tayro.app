...
describe('DeleteAccountModal', () => {
  ...
  it('sucesso chama DELETE /influencers/me, limpa a sessão e navega pra "/"', async () => {
    ...
    await waitFor(() =>
      expect(api.delete).toHaveBeenCalledWith('/influencers/me', {
        data: { password: 'senhaAtual1' },
      }),
    );
  ...
  it('mostra "sem conexão" apenas quando a request não chega ao servidor', async () => {
    vi.mocked(api.delete).mockRejectedValue({ isAxiosError: true });
    renderModal();

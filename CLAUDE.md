...
- **Ciclo de vida do programa (v0.34.0):** os 4 endpoints (`PATCH /campaigns/:id`, `/publish`, `/close`, `DELETE /campaigns/:id`) existiam desde o começo e nenhum tinha tela — auditoria de 2026-08-13 (varredura de TODA rota da API contra o que o front consome). Dois furos reais, não só telas faltando: (a) rascunho salvo com "Agora não" ficava **preso em DRAFT pra sempre** — `usePublishCampaign` só era chamado no modal pós-criação do `NewCampaignPage`; (b) `close` era o único writer de `CLOSED` no código inteiro, então **nenhum programa jamais chegou a CLOSED em prod** (aba "Encerradas" estruturalmente vazia). Agora `CampaignActions` no `CampaignDetailPage` deriva as ações do status (DRAFT → Publicar/Editar/Excluir · ACTIVE → Encerrar · CLOSED/COMPLETED → nenhuma), cada transição irreversível passa por confirmação que diz o que acontece (não "tem certeza?"), e o modal **fica aberto no erro** pra dar retry. `CampaignForm` extraído do `NewCampaignPage` e compartilhado com o novo `EditCampaignPage` (`/brand/campaigns/:id/edit`); schema + conversores centavos↔R$ e ISO↔yyyy-MM-dd em `campaignFormSchema.ts` (arquivo de componente não pode exportar helper — `react-refresh/only-export-components` quebra o lint). StatusPill no header do detalhe. Só DRAFT edita: a tela não tenta salvar o que a API recusa com 400, explica o motivo.
  - **Fix de cache junto:** existiam DUAS `campaignKeys` exportadas com shapes diferentes — `useCampaigns.ts` tinha `detail: ['campaigns', id]` (plural) e `useCampaignApplications.ts` tem `detail: ['campaign', id]` (singular, a que o `useCampaign` realmente lê). O invalidate do publish acertava uma chave que nenhuma query lia; não aparecia porque o publish navegava e remontava a tela. Com publicar/encerrar rodando dentro do próprio detalhe isso viraria bug visível (status velho na tela). A `detail` duplicada foi removida do `useCampaigns.ts`.
- **Bugs de UI morta corrigidos (v0.34.0):** botão "Esqueci" do `LoginPage` era clicável em prod e não fazia nada (sem `onClick`, sem rota, sem endpoint de recuperação de senha) — removido, com teste de regressão; volta junto com o fluxo real. Copy do `PublishModal` prometia "editar os detalhes a qualquer momento" — mentira, o backend só edita DRAFT.
- **Footgun `z.coerce.number()` + campo opcional (JÁ NOS MORDEU, v0.34.0):** input numérico vazio chega como `""` (o RHF lê do DOM), `z.coerce.number()` converte `""` em `0`, e `.min(1)` estoura. `.optional()` NÃO salva — ele só aceita `undefined`, nunca `""`. Efeito em produção: "Prazo p/ pagamento (dias)", opcional na API (`@IsOptional()`), **travava a criação de programa** com "Number must be greater than or equal to 1" (em inglês, num campo sem asterisco). Bug pré-existente, descoberto só ao cobrir a criação com teste — o form nunca tinha tido spec. Fix: helper `optionalPositiveInt` com `z.preprocess` mapeando `""`/`null` → `undefined`, em `campaignFormSchema.ts`. **Todo campo numérico opcional novo tem que usar o preprocess**, senão repete.
- Estrutura de testes (Jest + Vitest): **148 testes API + 161 web** (2026-08-13)
...
- **Furos de ponta a ponta que sobraram da auditoria de 2026-08-13** (escopo cortado de propósito da v0.34.0, que fechou só o ciclo de vida do programa):
  - **Recuperação de senha não existe** (nem `POST /auth/forgot-password` nem tela). Quem esquece a senha fica fora do produto pra sempre — `POST /auth/claim` define senha uma vez e acabou. O botão "Esqueci" do mock foi REMOVIDO do Login até o fluxo existir. Bloqueado de fato por e-mail: `EMAIL_PROVIDER=stub` em prod (sem domínio verificado no Resend), então o e-mail não chegaria em ninguém.
  - **Sem trocar senha/e-mail logada, sem deletar conta nem exportar dados** (LGPD — pesa mais aqui porque o produto já se posiciona nisso com o `publicProfileEnabled` opt-in).
  - **Marca não descobre creators.** CRM "creator-first" sem busca/listagem de creators pra marca: ela só vê quem se candidatou. `/c/:handle` existe mas só é alcançável por link direto — e o próprio `ProfilePage` da creator mostra `tayro.app/c/{handle}` como TEXTO, não link clicável.
  ...
  - **`PATCH /applications/:id/withdraw` é só parcialmente alcançável:** o botão só existe na candidatura PENDING em destaque do `MyApplicationsPage`; as linhas da lista abaixo são inertes. Creator com 3 PENDING só consegue retirar a mais recente.
- **Dívida de modelo de dados sem superfície (auditoria 2026-08-13) — decisão: NÃO mexer por ora, dropar tabela é irreversível e não urge:**
  - **`Notification`** — tabela com índice composto e ZERO referências em todo o código (API e web). Migration morta.
  - **`PartnershipResult`** — meio ligado, o que confunde mais que não ligado: é lido em `getPublicProfile` e devolvido como `results[]`, está tipado em `types/api.ts`, e o `PublicCreatorProfilePage` nunca renderiza. Não existe endpoint de escrita → o array é sempre `[]` e `visibleToCreator` nunca é setada por ninguém.
...
- **/brand/campaigns/:id/edit (v0.34.0):** EditCampaignPage — mesmo `CampaignForm` do "novo", prefill via `campaignToFormValues`. Fora de DRAFT não mostra form, mostra o motivo + volta pro detalhe.
